...
`.github/workflows/sync-umapyoi.yml` runs the JP, Global website, and Global social syncs every six hours and commits the archives only when source content changed. The GitHub-hosted Global website fetch uses `news.tnnlb.dev` as a reverse proxy for the official API because the official site blocks GitHub Actions runners. It can also be started manually with the full-refresh option for the JP source.
