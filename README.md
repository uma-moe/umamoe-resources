...
`.github/workflows/sync-umapyoi.yml` runs the JP, Global website, and Global social syncs every six hours and commits the archives only when source content changed. Because the official site blocks GitHub Actions runners, the Global website step routes its official API request through the outbound HTTPS proxy stored in the `PROXY_URL` Actions secret. It can also be started manually with the full-refresh option for the JP source.
